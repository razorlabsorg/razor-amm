#[test_only]
module razor_amm::amm_router_tests {
  use std::signer;
  use std::vector;
  ...
  fun test_add_liquidity(
    ...
    amm_router::add_liquidity(alice, token0_address, token1_address, alice_liquidity_x, alice_liquidity_y, 0, 0, @alice, 1000000000);

    let pair = amm_pair::liquidity_pool(token0, token1);

    let (reserve_x, reserve_y, _) = amm_pair::get_reserves(pair);

    let pool_lp_balance = amm_pair::lp_balance_of(@razor_amm, pair);
    let bob_lp_balance = amm_pair::lp_balance_of(@bob, pair);
    let alice_lp_balance = amm_pair::lp_balance_of(@alice, pair);
  ...
  fun test_add_liquidity_with_less_x_ratio(
    ...
    amm_router::add_liquidity(bob, token0_address, token1_address, bob_add_liquidity_x, bob_add_liquidity_y, 0, 0, @bob, 1000000000);
    ...
    let bob_lp_balance = amm_pair::lp_balance_of(@bob, pair);
    let deployer_lp_balance = amm_pair::lp_balance_of(@razor_amm, pair);
  ...
  #[test(admin = @admin, deployer = @razor_amm, bob = @bob, alice = @alice)]
  #[expected_failure(abort_code = 12, location = razor_amm::amm_router)]
  fun test_add_liquidity_with_less_x_ratio_and_less_than_y_min(
    ...
    amm_router::add_liquidity(bob, token0_address, token1_address, initial_reserve_x, initial_reserve_y, 0, 0, @bob, 1000000000);

    let bob_add_liquidity_x = 1 * pow(10, 8);
    let bob_add_liquidity_y = 5 * pow(10, 8);

    amm_router::add_liquidity(bob, token0_address, token1_address, bob_add_liquidity_x, bob_add_liquidity_y, 0, 4 * pow(10, 8), @bob, 1000000000);
  }

  #[test(admin = @admin, deployer = @razor_amm, bob = @bob, alice = @alice)]
  #[expected_failure(abort_code = 11, location = razor_amm::amm_router)]
  fun test_add_liquidity_with_less_y_ratio_and_less_than_x_min(
    ...
    amm_router::add_liquidity(bob, token0_address, token1_address, bob_add_liquidity_x, bob_add_liquidity_y, 5 * pow(10, 8), 0, @bob, 1000000000);
  }
  ...
  fun test_remove_liquidity(
    ...
    amm_router::add_liquidity(bob, token0_address, token1_address, token0_amount, token1_amount, 0, 0, @bob, 1000000000);

    let pair = amm_pair::liquidity_pool(token0, token1);

    amm_router::remove_liquidity(bob, token0_address, token1_address, expected_liquidity - MINIMUM_LIQUIDITY, 0, 0, @bob, 1000000000);
    ...
    let lp_total_supply = amm_pair::lp_token_supply(pair);
  ...
  fun test_swap_tokens_for_exact_tokens(
    ...
    amm_router::swap_tokens_for_exact_tokens(
      bob,
      amount_out,
      max_amount_in,
      path,
  ...
  #[test(admin = @admin, deployer = @razor_amm, bob = @bob)]
  #[expected_failure(abort_code = 1, location = razor_amm::amm_router)]
  ...
  #[test(admin = @admin, deployer = @razor_amm, bob = @bob)]
  #[expected_failure(abort_code = 3, location = razor_amm::amm_router)]
