module razor_amm::swap_library {
  ...
  public fun get_reserves(
    ...
    let token0_addr = object::object_address(&token0);
    let token1_addr = object::object_address(&token1);
    let (reserve0, reserve1) = get_reserves(token0_addr, token1_addr);
  ...
  public fun get_amounts_out(
    ...
    let amounts = vector::singleton(amount_in);
  ...
  public fun calc_optimal_coin_values(
    ...
    assert!(amount_a_min <= amount_a_desired && amount_b_min <= amount_b_desired, ERROR_INSUFFICIENT_AMOUNT);
    let (token_a, token_b) = sort_tokens(token0, token1);
    let token_a_addr = object::object_address(&token_a);
    let token_b_addr = object::object_address(&token_b);
    let (reserve_a, reserve_b) = get_reserves(token_a_addr, token_b_addr);
    ...
    if (reserve_a == 0 && reserve_b == 0) {
      ...
      if (amount_b_optimal <= amount_b_desired) {
        assert!(amount_b_optimal >= amount_b_min, ERROR_INSUFFICIENT_B_AMOUNT);
        ...
        assert!(amount_a_optimal >= amount_a_min, ERROR_INSUFFICIENT_A_AMOUNT);
